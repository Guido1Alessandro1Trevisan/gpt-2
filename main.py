...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # attention (materializes the large T, T) matrix for all queries
...
if ddp:
    assert torch.cuda.is_available(), "from now on i think we need to use CUDA"
...
if master_process:
    ...
    print(f"=> calculated gradient descent: {grad_accum_steps}")
...
for step in range(max_steps):
    ...
    print(f"step {step} loss: {loss_accum.item()} dt:{dt:.2f}ms token per sec{token_per_sec:.2f} norm: {norm:.4f}")
